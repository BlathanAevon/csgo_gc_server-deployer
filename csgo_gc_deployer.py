...
def create_sourcemod_admins_simple(cfg: DeployConfig) -> str:
    lines = [
        '// SourceMod admins_simple generated by deployer',
        '// Format: "identity" "flags" // comment',
        '// Full root admin flags: 99:z',
        '',
    ]
    if cfg.admin_steamid:
        lines.append(f'"{cfg.admin_steamid}" "99:z" // Primary server administrator')
    else:
        lines.extend([
            '// No admin SteamID configured yet.',
            '// Add one line like:',
            '// "STEAM_1:1:123456" "99:z" // YourName',
        ])
    lines.append('')
    return "\n".join(lines)
...
def preinstall_commands(cfg: DeployConfig) -> Iterable[tuple[str, str | None]]:
    ...
    yield (
        "apt-get install -y "
        "lib32gcc-s1 lib32stdc++6 lib32z1 screen tmux tar wget unzip",
        None,
    )
    yield (
...
def addon_commands(cfg: DeployConfig) -> Iterable[tuple[str, str | None]]:
    ...
    # Ensure core admin panel components are enabled even if a package places them in disabled/.
    yield (
        f"cd {shlex.quote(str(game_dir / 'addons' / 'sourcemod'))} && "
        "for p in adminmenu.smx admin-flatfile.smx basecommands.smx; do "
        "test -f plugins/disabled/$p && mv plugins/disabled/$p plugins/$p || true; "
        "done",
        cfg.steam_user,
    )
...
def _validate_steam2_id(value: str) -> bool:
    return re.fullmatch(r"STEAM_[0-5]:[01]:\d+", value) is not None


def _ask_steam2_id(label: str, default: str = "", hint: str = "") -> str:
    while True:
        value = _ask(label, default=default, hint=hint)
        if not value:
            return ""
        if _validate_steam2_id(value):
            return value
        print(_c("  ✗ Use Steam2 format, e.g. STEAM_1:1:123456", _RED))
...
def wizard() -> DeployConfig:
    ...
    _info("SourceMod admin panel stack is enabled by default for frictionless administration.")
...
def _post_start_self_test(cfg: DeployConfig, attempts: int = 20, delay_sec: float = 1.0) -> None:
    """Validate that the server process is alive and session is present after startup."""
    _header("Post-start self-test")
    _info("Validating server process and detached session...")

    for attempt in range(1, attempts + 1):
        proc = subprocess.run(
            ["su", "-", cfg.steam_user, "-c", "pgrep -f 'srcds_linux|srcds_run|csgo_gc|hl2_linux' >/dev/null 2>&1"],
            check=False,
        )
        if proc.returncode == 0:
            break
        if attempt < attempts:
            _info(f"Server process not ready yet (attempt {attempt}/{attempts}); retrying...")
            time.sleep(delay_sec)
    else:
        raise RuntimeError("Post-start self-test failed: no running server process detected.")
    ...
    session_check = subprocess.run(
        [
            ...
            cfg.steam_user,
            "-c",
            "tmux has-session -t csgo >/dev/null 2>&1 || screen -ls | grep -q '[.]csgo\\b'",
        ],
        check=False,
    )
...
def deploy(cfg: DeployConfig) -> bool:
    ...
    print(f"     rcon_password \"{cfg.rcon_password}\"")
    print("     rcon status")
    _info("RCON is intended for in-game client console use in this setup.")
    ...
    print(f"     su - {cfg.steam_user} -c '{cfg.install_dir}/admin.sh panel'")
    if cfg.install_sourcemod_stack:
        print()
